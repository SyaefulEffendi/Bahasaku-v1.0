...
def upload_profile_photo(user_id):
    ...
    # Keep track of old file to delete after successful commit
    user = User.query.get_or_404(user_id)
    old_url = user.profile_pic_url or ''
    old_filename = None
    if old_url and ('/foto_profile/' in old_url):
        # old_url may be absolute or relative
        old_filename = old_url.split('/foto_profile/')[-1]

    # Update model to point to new URL and commit
    try:
        user.profile_pic_url = absolute_url
        db.session.commit()

        # If commit succeeded, delete old file (but don't delete if same name)
        if old_filename:
            try:
                old_path = os.path.join(current_app.root_path, 'static', 'foto_profile', old_filename)
                # Safety: ensure old_path is inside the foto_profile directory
                ...
                # Non-fatal: if delete fails, log and continue
                pass

        return jsonify({"message": "Foto profil berhasil diupload", "user": user.to_profile_dict()}), 200

    except Exception as e:
        # Commit failed: remove the newly saved file to avoid orphan files
        try:
...
# Rute GET by ID (Mendapatkan Profil User Tertentu)
